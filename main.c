...
/* Main routine of the process manager. */
  ...
  while (TRUE) {
	  ...
	  if(who_p >= 0 && mp->mp_endpoint != who_e && (call_nr != 118)) {
...
void unblockedsyscalls()
{
    register struct mproc *myproc = NULL;
    int value;
    ...
    if(member!=NULL)
    {
        ...
        myproc = &mproc[member->publisher]; 
        myproc->mp_flags |= UNPAUSED;
        who_p = member->publisher;
        call_nr = 118;  /* do_igpublish */
         
        value = (*call_vec[call_nr])();

        if (value != SUSPEND) 
        {

            setreply(who_p, value);
        }
    }
    else
    {
        printf("%s : %d something has really gone wrong\n",__FILE__,__LINE__);
    }
}
